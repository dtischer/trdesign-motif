...
def mk_xml(weight_file, min_aa_probability, sequnce_profile_weight, layer_design, pssm_f_updated, hbnet_resnums_str, args, cart=False):
  pack_protocol = f"""
  <ROSETTASCRIPTS>
    <SCOREFXNS>
    ...
    <TASKOPERATIONS>
        ...
        {'</OperateOnResidueSubset>' if hbnet_resnums_str is not None else ''}
    ...
    <MOVERS>
      ...
      <FastDesign name="fastDesign" scorefxn="SFXN3" repeats="2"  task_operations="init,ex1_ex2aro,{'ld_surface_not_hbnets,fix_hbnet_residues,' if hbnet_resnums_str is not None else ''}ic,limitchi2,pssm_cutoff,noCys,repack_hotspots{',repack_interface_chainB,freeze_not_interface_chainB' if args.tar else ''}{",strand_surface_aa,strand_boundary_aa,strand_core_aa,design_loops_surface,design_loops_boundary,design_loops_core,design_edges_core,ld1,ld2,ld5,ld6,ld9,ld10,ld13" if layer_design else ""}" batch="false" ramp_down_constraints="false" cartesian="{str(cart).lower()}" bondangle="false" bondlength="false" min_type="dfpmin_armijo_nonmonotone" relaxscript="MonomerDesign2019"> 
          <MoveMap name="MM">                
              <ResidueSelector selector="chainA" chi="true" bb="true" bondangle="false" bondlength="false" />
              {'<ResidueSelector selector="interface_chainB" chi="true" bb="false" bondangle="false" bondlength="false" />' if args.tar else ''}
              {'<ResidueSelector selector="not_interface_chainB" chi="false" bb="false" bondangle="false" bondlength="false" />' if args.tar else ''}
...
def main():
    ...
    print('asdfas;fjsadlfjk;')
    print(hb_by_res.empty)
    
    if not hb_by_res.empty:    
    ...
    # Run design
    ###########################################################################################
    if 'hbnet_resnums' in locals():
        hbnet_resnums_str = ','.join([str(x) for x in hbnet_resnums])
    else:
        hbnet_resnums_str = None
    
    xml_rd1 = mk_xml(weight_file, min_aa_probability, sequnce_profile_weight, args.layer_design, pssm_f_updated, hbnet_resnums_str, args, cart=True)  # just a string replacement operation
