...
# from a PDB file
def parse_pdb(filename, contigs=None):
    ...
    xyz = np.stack([N,Ca,C,Cb], axis=0)

    # indices of residues observed in the structure
    idx = np.array([int(l[22:26]) for l in lines if l[:4]=="ATOM" and l[12:16].strip()=="CA"])

    if contigs is not None:
        ranges =  [np.array(x.split('-')).astype(int) for x in contigs.split(',')]
        idx_contig = np.concatenate([np.arange(y[0],y[1]+1) for y in ranges])
        mask = np.isin(idx,idx_contig)
        idx = idx[mask]
        xyz = xyz[:,mask,:]
    # rosetta energy
    #e = 0.0
    ...
    #    if l[:4]=='pose':
    #        e = float(l.split()[-1])
    #e = [l.split()[-1] for l in lines if l[:4]=='pose']
