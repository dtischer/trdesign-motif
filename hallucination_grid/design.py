...
BKG_DIR = script_dir+'/../db/grid_model/bkg/'
...
def main(argv):
    ...
    p.add_argument('--len', '-l', type=int, default=100,  help='sequence length')
    ...
    with open(script_dir+'/../db/grid_model/params.json') as jsonfile:
        params = json.load(jsonfile)

    # load network weights
    weights = load_weights(script_dir+'/../db/grid_model/models',params, o.n_models)
    ...
    for L in L_range:
        ...
        if os.path.exists(fn):
            ...
            print(f"generating background len={L}... ",end='')
            for i in range(NRUN_BKG):
                pssm = np.random.normal(0,o.init_sd,size=(NSEQ_BKG,L,20))
                x_inp = argmax(pssm)
                p2d_ = sess.run(p2d, feed_dict={x:x_inp,drop_rate:0.0})
                bkg_np += np.mean(p2d_,axis=0)
            bkg_np /= NRUN_BKG
            print(f' saving to {fn}')
            np.savez(fn, bkg=bkg_np)
        bkg[L] = bkg_np
